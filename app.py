...
# Telegram exposes no line limit, so a character budget is the only lever and
# the rendered line count varies with client width. The preview is collapsed to
# a single paragraph first, so a hard-wrapped body cannot arrive as a column of
# stubs or push the link off the bottom of the notification.
...
def build_message(event: str, convo: dict) -> str:
    ...
    if subject:
        lines.append(f"<b>{esc(subject)}</b>")

    # split()/join collapses newlines and runs of spaces, so the body is one
    # paragraph before it is cut -- otherwise a hard-wrapped email arrives as a
    # column of stubs.
    preview = " ".join((convo.get("preview") or "").split())
    if preview:
