...
<!-- TODO: screenshot of a delivered Telegram notification -->
<p align="center"><em>screenshot placeholder</em></p>
...
- A FreeScout install with the **API & Webhooks** module enabled. It is
  **bundled and free** in recent FreeScout versions — you do not need to buy
  anything. (`Manage → Modules → API & Webhooks → Activate`.)
...
| `PREVIEW_CHARS` | no | `300` | Body preview truncation length. |
...
docker run -d --name freescout-telegram --env-file .env \
  ghcr.io/cloudlineas/freescout-telegram-bridge:v0.1.0
```
