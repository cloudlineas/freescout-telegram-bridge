...
def test_subject_is_bold_and_unlabelled():
    text = bridge.build_message("convo.created", CONVO)
    assert "<b>Cannot log in</b>" in text
    assert "Subject:" not in text


def test_missing_subject_is_omitted():
    convo = {k: v for k, v in CONVO.items() if k != "subject"}
    text = bridge.build_message("convo.created", convo)
    assert "<b></b>" not in text


def test_multiline_preview_is_collapsed_to_one_paragraph(monkeypatch):
    """A hard-wrapped email must not arrive as a column of stubs."""
    monkeypatch.setattr(bridge, "PREVIEW_CHARS", 300)
    body = "First line\nsecond line\n\n   third   line  "
    text = bridge.build_message("convo.created", dict(CONVO, preview=body))
    assert "<i>First line second line third line</i>" in text
    assert "\n" not in text.split("<i>")[1].split("</i>")[0]


def test_collapsed_preview_is_measured_after_collapsing(monkeypatch):
    """Whitespace must not eat into the character budget."""
    monkeypatch.setattr(bridge, "PREVIEW_CHARS", 11)
    text = bridge.build_message("convo.created", dict(CONVO, preview="one\n\n\ntwo three four"))
    # 11 chars of the collapsed text, not of the raw body -- the newlines would
    # otherwise consume a third of the budget. Truncation is mid-word.
    assert "<i>one two thr…</i>" in text


def test_preview_default():
    assert bridge.PREVIEW_CHARS == 160
